--1.
--Write a query using CONCAT() to the Application.Cities table,
--putting together CityName and LatestRecordedPopulation, separating the two fields with a hyphen.

SELECT CONCAT(CityName, '-', LatestRecordedPopulation) AS 'City/Pop'
FROM Application.Cities

--Notice how CityName is a string (nvarchar(50)) and LatestRecordedPopulation is a BIGINT. CONCAT() handles the
--conversion of types for you. More on data types later.
...
--Accomplish the following in your query:
    --(a) use CONTCAT() and concatenate Fullname, and SearchName
...
--4. Write a query to the Application.Cities table, returning only CityName, LatestRecordedPopulation
